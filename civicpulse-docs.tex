...
\centering
\begin{tikzpicture}[
    issue/.style={circle, fill=cpblue, inner sep=0pt, minimum size=6pt},
    newissue/.style={circle, fill=cpteal, inner sep=0pt, minimum size=8pt, drop shadow={opacity=0.3}},
    radius/.style={circle, draw=cpteal!50, fill=cpteal!10, inner sep=0pt, minimum size=4cm}
  ]
  % Radius
  \node[radius] at (0,0) {};
  
  % Center
  \node[newissue, label=above:{\footnotesize New Issue}] at (0,0) {};
  
  % Existing issues
  \node[issue, label=right:{\footnotesize Sim: 0.92}] at (0.8, 0.5) {};
  \node[issue, label=left:{\footnotesize Sim: 0.88}] at (-1.2, 0.2) {};
  \node[issue, label=below:{\footnotesize Sim: 0.75}] at (0.4, -1.1) {};
  
  \node[font=\sffamily\small\color{cpteal}, align=center] at (0, -2.4) {PostGIS 200m Search Radius};
  
  \draw[dashed, cpteal] (0,0) -- (0.8, 0.5);
  \draw[dashed, cpteal] (0,0) -- (-1.2, 0.2);
  \draw[dashed, cpteal] (0,0) -- (0.4, -1.1);
\end{tikzpicture}
\caption{Agent 2 queries PostGIS for issues within 200m, then applies pgvector cosine similarity. Issues $\ge 0.85$ are flagged as duplicates.}
\end{figure}
\label{sec:agent2}
...
\screenshot[0.7\linewidth]{s10_agent5.jpg}{Agent 5 predictive alerts panel}
\label{sec:agent5}
...
\subsection{Google Antigravity \& MCP Tooling}
During development, the platform was built and accelerated using Google's advanced AI coding ecosystem, specifically the \textbf{Antigravity IDE} and \textbf{Antigravity Agents} (including Agent Ali). To enable autonomous browser testing and context-aware development, the \textbf{Google Stitch MCP Server} and the \textbf{Chrome DevTools MCP Server} were heavily leveraged, allowing the AI to seamlessly interact with local development servers and debug web interfaces in real time.
...
\centering
\begin{tikzpicture}[
    point/.style={circle, fill=cpblue, inner sep=0pt, minimum size=8pt, drop shadow={opacity=0.3}},
    depot/.style={rectangle, rounded corners=2pt, fill=cpteal, text=white, font=\sffamily\bfseries\footnotesize, inner sep=3pt, drop shadow={opacity=0.3}}
  ]
  
  \node[depot] (D) at (0,0) {Base};
  \node[point] (P1) at (1, 2) {};
  \node[point] (P2) at (3, 2.5) {};
  \node[point] (P3) at (4, 0.5) {};
  \node[point] (P4) at (2.5, -1) {};
  
  \draw[->, thick, cpblue] (D) -- (P1) node[midway, left, font=\scriptsize\color{cpgray}] {1};
  \draw[->, thick, cpblue] (P1) -- (P2) node[midway, above, font=\scriptsize\color{cpgray}] {2};
  \draw[->, thick, cpblue] (P2) -- (P3) node[midway, right, font=\scriptsize\color{cpgray}] {3};
  \draw[->, thick, cpblue] (P3) -- (P4) node[midway, right, font=\scriptsize\color{cpgray}] {4};
  \draw[->, dashed, cpblue!50] (P4) -- (D) node[midway, below, font=\scriptsize\color{cpgray}] {Return};
  
\end{tikzpicture}
\caption{TSP Route Optimisation computes the most efficient path for municipal workers to address clustered issues in a single trip.}
\end{figure}
...
\subsection{Interactive Geospatial Visualisation}

\screenshot[0.7\linewidth]{s9_admin_map.jpg}{Interactive Geospatial Map — Real-time issue clustering for citizens and administrators}

CivicPulse leverages Mapbox GL JS and GeoJSON to render an interactive, real-time map of all civic issues across the region. Unlike traditional platforms that restrict geospatial data to administrators, this interactive clustering map is exposed to \textbf{both citizens and administrators}.
...
\subsection{Loading Animations \& Pipeline Visibility}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Dynamic pipeline UI showing progress through Deduplication, Validation, and Resolution.}
\end{figure}
...
\subsection{Web Accessibility (a11y) \& Screen Reader Support}
...
\begin{itemize}
  \item \textbf{Screen Reader Labels:} All icon-only interactive elements (like the ``Use GPS location'' and ``Choose location on map'' buttons) are explicitly tagged with \tech{aria-label}, ensuring screen readers articulate the exact action rather than skipping the icon.
  \item \textbf{Tab Navigation \& Roles:} Complex UI components like dashboard status/category filters use strict WAI-ARIA patterns including \tech{role="tablist"}, \tech{role="group"}, \tech{aria-selected}, and \tech{aria-pressed}. This allows users to tab through filters and intuitively understand the active state entirely via keyboard navigation.
  \item \textbf{Dynamic Announcements:} For asynchronous background tasks (like the AI pipeline processing), visually hidden \tech{sr-only} containers with \tech{aria-live="polite"} and \tech{aria-atomic="true"} are used. This allows the UI to silently announce status changes (e.g., ``Issue deduplication complete'') to screen readers without jarring visual interruptions.
\end{itemize}
\clearpage
...
\subsection{Agentic Retry Logic \& Self-Correction}
...
\begin{itemize}
  \item \textbf{Syntax Level (Self-Correction):} If an agent outputs invalid JSON or hallucinates extra text, the \tech{generateStructuredJSON} function catches the \tech{SyntaxError}, feeds the exact error message back into a new prompt, and asks the LLM to self-correct its output (up to 3 attempts).
  \item \textbf{API Level (Exponential Backoff):} Temporary API rate limits or network timeouts are handled via exponential backoff retries within the AI client.
  \item \textbf{State Level (LangGraph Resume):} If all in-node agentic retries fail, the LangGraph pipeline safely halts and persists the state as \tech{*\_failed} in the database. When triggered via the \file{/api/reports/retry} endpoint, the pipeline reconstructs the full \tech{AgentState} and resumes precisely from the failed agent, ensuring that expensive previous steps (like image analysis or embeddings) are never re-run unnecessarily.
\end{itemize}
...
\screenshot[0.7\linewidth]{s2_login.jpg}{Login — Google OAuth + email magic link}
...
\screenshot[0.7\linewidth]{s7_community_review.jpg}{Community verification — verify/deny panel}
...
\screenshot[0.7\linewidth]{s3_report_top.jpg}{Report page — text input + voice button}

\screenshot[0.7\linewidth]{s5_dashboard.jpg}{Citizen dashboard — issue cards with pipeline stepper}
...
\screenshot[0.7\linewidth]{s8_admin_inbox.jpg}{Admin dashboard — issue inbox with filters}
